...
module Palette {
    ...
    function get(index as Number) as Dictionary {
        var table = SCREEN_IS_AMOLED ? TABLE_AMOLED : TABLE_MIP;
        var i = (index >= 0 and index < table.size()) ? index : 0;
        return table[i];
    }

    function size() as Number {
        return TABLE_AMOLED.size();
    }

    function nameFor(index as Number) as String {
        var i = (index >= 0 and index < NAME_IDS.size()) ? index : 0;
        return Application.loadResource(NAME_IDS[i]);
    }
